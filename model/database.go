...
func InitDB(address, dbName string) *RdbUtils {
...
func (db *RdbUtils) DeviceUpdateOrInsert(dev proto.DeviceInfo) error {
	dev.Present = util.NewBool(true)
	dev.PresenceChangedAt = time.Now()
	// only update when create
	dev.Ready = util.NewBool(false)
	dev.Using = util.NewBool(false)
